...
SELECT	M1.firstname AS member,
		M1.surname AS member_surname,
		M2.firstname AS recommended,
		M2.surname AS recommended_surname

FROM	Members AS M1 INNER JOIN Members M2 ON M1.memid = M2.recommendedby
WHERE	M1.recommendedby != ''
ORDER BY M2.firstname, M2.surname


/* Q12: Find the facilities with their usage by member, but not guests */

SELECT	F.name,
		COUNT(B.starttime) * .5 AS Total_hours
...
SELECT	F.name,
		DATE_FORMAT(B.starttime, "%m") AS month,
		COUNT(B.starttime) * .5 AS total_hours
